...
Utils.getId().then(res=>{})
var Voting = new web3.eth.Contract(voting_artifacts.abi, voting_artifacts.networks[1675325694502].address);
let candidates = {};
...
function populateTokenData() {
    ...
    web3.eth.getBalance(voting_artifacts.networks[1675325694502].address).then(balance => {
...
window.buyTokens = function () {
    ...
    Voting.methods.buy().send({ from:"0x8043ed3Cd40E86EEeB5965FA58beBB1D3d87De63", value: _value }).then(() => {
        Voting.methods.totalSold().call().then(amount => {
            $("#tokens-sold").html(amount.toString());
        });
        web3.eth.getBalance(voting_artifacts.networks[1675325694502].address).then(balance => {
...
window.voteForCandidate = function () {
    ...
    Voting.methods.voteForCandidate(candidateName, voteTokens).send({ from: "0x8043ed3Cd40E86EEeB5965FA58beBB1D3d87De63",gas:999999}).then(() => {
